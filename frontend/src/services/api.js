...
export async function createSurvey(formData) {
  try {
    ...
    const name =
      formData.get("name") || "Uploaded Sonar Scan";
    ...
    const sonar_type =
      formData.get("sonar_type") ||
      "Side-Scan Sonar (SSS)";

    const start_lat =
      parseFloat(formData.get("start_lat")) || 18.9220;

    const start_lon =
      parseFloat(formData.get("start_lon")) || 72.8347;
...
export async function fetchDetections(filters = {}) {
  try {
    ...
    if (filters.survey_id)
      query.append("survey_id", filters.survey_id);

    if (
      filters.class_name &&
      filters.class_name !== "all"
    )
      query.append("class_name", filters.class_name);

    if (
      filters.risk_level &&
      filters.risk_level !== "all"
    )
      query.append("risk_level", filters.risk_level);
    ...
    console.warn(
      "API Offline, using fallback sample detections:",
      err
    );
    ...
    const fallbackList = [
      {
        id: "det-mumbai-01",
        survey_id: "surv-mumbai-001",
        class_name: "ghost_net_fishing_gear",
        confidence: 0.92,
        bbox: [0.225, 0.300, 0.18, 0.14],
        lat: 18.9241,
        lon: 72.8316,
        risk_score: 0.86,
        risk_level: "Critical",
      ...
      {
        id: "det-vizag-01",
        survey_id: "surv-vizag-002",
        class_name: "shipwreck",
        confidence: 0.98,
        bbox: [0.300, 0.250, 0.28, 0.22],
        lat: 17.6883,
        lon: 83.2217,
        risk_score: 0.93,
        risk_level: "Critical",
      ...
      {
        id: "det-gulf-01",
        survey_id: "surv-gulf-003",
        class_name: "ghost_net_fishing_gear",
        confidence: 0.95,
        bbox: [0.280, 0.340, 0.22, 0.16],
        lat: 9.2888,
        lon: 79.3101,
        risk_score: 0.91,
        risk_level: "Critical",
    ...
    return fallbackList.filter(d => {
      ...
      if (
        filters.class_name &&
        filters.class_name !== "all" &&
        d.class_name !== filters.class_name
      )
        return false;
      ...
      if (
        filters.risk_level &&
        filters.risk_level !== "all" &&
        d.risk_level !== filters.risk_level
      )
        return false;
      ...
      if (
        filters.min_confidence &&
        d.confidence < filters.min_confidence
      )
        return false;
...
export async function verifyDetection(
  ...
  try {
    ...
    if (!res.ok)
      throw new Error("Failed to verify detection");
    ...
    return {
      ...
      status:
        action === "reject"
          ...
          : "confirmed",
      class_name:
        newClass || "ghost_net_fishing_gear",
      note
    };
  }
}
...
export function getImageUrl(path) {
  if (!path)
    return "/samples/sample_mumbai_raw.jpg";
...
export async function fetchResearchMetrics() {
  try {
    ...
    if (!res.ok)
      throw new Error(
        "Failed to fetch research metrics"
      );

    return await res.json();
  } catch (err) {
    console.warn(
      "API Offline or metrics.json missing:",
      err
    );
